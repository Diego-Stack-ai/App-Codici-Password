...
Censimento in `docs/A3_CENSIMENTO_UTENZE.md`. A3-R2 completa nel laboratorio l'editor delle utenze private annidato sotto il relativo indirizzo A2: sorgente revocabile, vista/provider, ID persistiti, create/update/delete, doppia conferma, scarto locale e rilettura confermata; soltanto `value` viene cifrato. Indirizzo, altre utenze, password legacy, campi sconosciuti e collegamenti restano preservati. La guardia Account e le identità instabili restano fail-closed; l'indirizzo incluso nel QR non blocca la singola utenza. Collega/Cambia/Scollega conserva l'origine composta e usa il servizio collegamenti già esistente. Aggiunti endpoint loopback, Rules candidate e suite Firestore Emulator; nessuno schema aziendale è stato inventato. Prove: **11 test mirati**, emulatore **1/1**, shell **712/712**, Chrome desktop/mobile **31/31**; Edge tentato ma termina prima di DevTools sulla macchina di prova. A3 è completo per il perimetro di laboratorio, non pubblicato; restano trasporto/Rules produttivi e dispositivo fisico. Nessun deploy, bump o dato reale.
