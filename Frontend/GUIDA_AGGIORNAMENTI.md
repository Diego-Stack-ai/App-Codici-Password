...
## Note modificabili dal dettaglio Account — candidata 13/09/2026

Base `80ec7837`, ramo `fix/account-inline-notes`. Nei dettagli privato/azienda il riquadro Note resta visibile soltanto se contiene testo. Il proprietario vede sempre un comando: Aggiungi nota quando vuoto, Modifica nota quando compilato. Un dialogo permette di salvare direttamente senza aprire il form completo; il testo viene aggiornato dopo conferma della transazione. Svuotando il testo torna il solo comando Aggiungi nota. L'editor del form completo continua a usare lo stesso campo `note`.

Il writer cifra sul dispositivo e aggiorna soltanto note/revision/updatedAt. Confronta revisione e valore memorizzato, rifiuta Account archiviati, dati non validi e destinatari in sola lettura; conflitti/errori conservano la bozza. Blocco, cambio sessione e navigazione chiudono il dialogo e impediscono azioni tardive. Gli Account legacy non marcati `_encrypted:true` richiedono prima il normale salvataggio dal form completo: non si cambia il flag globale cifrando soltanto la nota, né si introducono nuove note in chiaro. Nessuna migrazione o deploy.
