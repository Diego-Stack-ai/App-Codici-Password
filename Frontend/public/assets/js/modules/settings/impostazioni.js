...
function setupSharedCredentials(user) {
    document.getElementById('btn-shared-credentials')?.addEventListener('click', async () => {
        try {
            const {openSharedCredentialsSettings} = await import('./shared-credentials-controller.js?v=1.2.128');
...
function setupAccountFieldUsage(user) {
    ...
    button.addEventListener('click', async () => {
        ...
        try {
            const {inspectAccountFieldUsage} = await import('./account-field-usage-service.js?v=1.2.128');
...
function setupAIAssistantToggle(user, data) {
    ...
    toggle.addEventListener('change', async () => {
        ...
        try {
            ...
            if (enabled) {
                const { initVaultAssistant } = await import('../assistant/assistant-controller.js?v=1.2.128');
