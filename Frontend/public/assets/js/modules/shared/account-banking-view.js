...
import { hasRealBankingData, normalizeBankingAccounts } from './banking-model.js?v=1.2.124';
...
function createBankAccount(bank, index, isActive) {
    ...
    const fields = [
        ...
        bank.numeroVerde ? field('Numero verde', bank.numeroVerde, 'support_agent') : null,
        bank.referenteNome ? field('Referente banca', bank.referenteNome, 'person') : null,
...
export function renderAccountBanking(account, { isReadOnly = false, onAddBanking, promptText, isActive = () => true } = {}) {
    ...
    setChildren(content, banks.map((bank, index) => createBankAccount(bank, index, isActive)));
}
