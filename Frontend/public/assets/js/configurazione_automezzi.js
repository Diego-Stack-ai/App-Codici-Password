        ...
        tr.innerHTML = `
             <td class="px-4 py-3 flex justify-between items-center text-gray-700 dark:text-gray-300">
                ...
                <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                    <button class="size-8 flex items-center justify-center rounded-lg hover:bg-blue-500/20 text-blue-600 dark:text-blue-500 transition-colors"
...
window.editType = async (index) => {
    ...
    const newName = prompt(t('prompt_edit_subject'), item.name);
...
window.addTypeItem = async () => {
    const name = prompt(t('prompt_new_subject'));
    if (!name) return;
    const period = prompt(t('prompt_period_default'), "14");
    if (period === null) return;
    const freq = prompt(t('prompt_freq_default'), "7");
    if (freq === null) return;
