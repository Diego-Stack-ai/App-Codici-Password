...
import { initComponents } from './components-v129.js?v=1.2.53';

import { LOG } from './logger.js';

export async function initAreaPrivata(user) {
    const module = await import('./modules/privato/area_privata.js?v=1.2.53');
...
export async function initIndex() {
    const module = await import('./modules/auth/login.js?v=1.2.53');
...
export async function initHomePage(user) {
    ...
    const module = await import('./modules/home/home.js?v=1.2.53');
...
export async function initProfiloPrivato(user) {
    const module = await import('./modules/privato/profilo_privato.js?v=1.2.53');
    await module.initProfiloPrivato(user);
}

export async function initImpostazioni(user) {
    const module = await import('./modules/settings/impostazioni.js?v=1.2.53');
...
export async function initAggiungiScadenza(user) {
    const module = await import('./modules/scadenze/aggiungi_scadenza.js?v=1.2.53');
