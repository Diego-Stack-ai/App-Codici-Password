...
function fixture(){let imports=0,resolve;const rendered=[],toasts=[];const gate=new Promise(r=>resolve=r);const state={formVersion:1,currentUid:'A',auth:{currentUser:{uid:'A'}},bankAccounts:[],accountWidgetController:{openNewWidget:()=>rendered.push({widget:true})},document:{getElementById:()=>({})},clearElement(){},showToast:message=>toasts.push(message),loadBankingRenderer:()=>{imports++;return gate}};vm.createContext(state);vm.runInContext(callback,state);return {state,rendered,toasts,get imports(){return imports},run:()=>vm.runInContext('rerender()',state),resolve:()=>resolve({renderBankAccounts:(data,rerender,options)=>rendered.push({data,rerender,options})})};}
...
test('bank widget shortcut is inert after navigation or logout',async()=>{
 for(const mode of ['navigation','logout']) {const f=fixture();f.state.bankAccounts=[{iban:'fixture'}];const pending=f.run();f.resolve();await pending;const action=f.rendered[0].options.onAddWidget;action();assert.equal(f.rendered.length,2);if(mode==='navigation')f.state.formVersion++;else f.state.auth.currentUser=null;action();assert.equal(f.rendered.length,2);}
});
