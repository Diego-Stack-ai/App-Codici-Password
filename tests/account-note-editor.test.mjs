...
function fixture() {
    const nodes = [], writes = [], toasts = [], events = new EventTarget(), observers = new Set();
    class Node {
...
test('Auth change closes the production editor without requiring the experimental page lifecycle', async () => {
    const f = fixture(); f.init(); await f.button.onclick(); f.input().value = 'Draft';
    f.changeUser(); assert.equal(f.input().value, ''); assert.equal(f.button.onclick, null);
    assert.equal(f.writes.length, 0);
});
