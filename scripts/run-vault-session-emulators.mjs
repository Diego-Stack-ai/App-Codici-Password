...
if (process.argv.length > 3 || process.argv.slice(2).some(arg => !['--browser', '--account-note', '--profile-link', '--profile-text', '--profile-contacts', '--profile-company-contacts', '--profile-addresses', '--profile-addresses-browser', '--profile-document-attachments', '--profile-document-attachments-storage', '--profile-document-attachments-browser', '--profile-company-contacts-browser', '--qr-selection', '--mutation', '--fenced-browser', '--entry-browser', '--cold-browser', '--restart-browser', '--crash-browser'].includes(arg))) throw new Error('INVALID_ARGUMENT');
