...
if (process.argv.length > 3 || process.argv.slice(2).some(arg => !['--browser', '--account-note', '--profile-link', '--profile-text', '--profile-contacts', '--profile-document-attachments', '--profile-document-attachments-storage', '--qr-selection', '--mutation', '--fenced-browser', '--entry-browser', '--cold-browser', '--restart-browser', '--crash-browser'].includes(arg))) throw new Error('INVALID_ARGUMENT');
