...
if (process.argv.length > 3 || process.argv.slice(2).some(arg => !['--browser', '--account-note', '--profile-link', '--profile-text', '--profile-contacts', '--profile-document-attachments', '--profile-document-attachments-storage', '--profile-document-attachments-browser', '--qr-selection', '--mutation', '--fenced-browser', '--entry-browser', '--cold-browser', '--restart-browser', '--crash-browser'].includes(arg))) throw new Error('INVALID_ARGUMENT');
...
const result = spawnSync(process.execPath, [cli, 'emulators:exec', '--project', 'demo-vault-shell',
    ...
    accountNote ? 'node --test firebase-account-note.test.mjs' : profileLink ? 'node --test firebase-profile-link.test.mjs' : profileText ? 'node --test firebase-profile-text.test.mjs' : profileContacts ? 'node --test firebase-profile-contacts.test.mjs' : profileDocumentAttachmentsBrowser ? 'node emulator-browser.mjs --test-attachments' : profileDocumentAttachmentsStorage ? 'node --test firebase-profile-document-attachments-storage.test.mjs profile-document-attachment-rules.test.mjs' : profileDocumentAttachments ? 'node --test firebase-profile-document-attachments.test.mjs' : qrSelection ? 'node --test firebase-qr-selection.test.mjs' : crashBrowser ? 'node emulator-browser.mjs --test-crash' : restartBrowser ? 'node emulator-browser.mjs --test-restart' : coldBrowser ? 'node emulator-browser.mjs --test-cold' : entryBrowser ? 'node emulator-browser.mjs --test' : fencedBrowser ? 'node ../offline-sync/run-emulated-browsers.mjs' : browser ? 'node emulator-browser.mjs' : mutation ? 'node --test firebase-mutation.test.mjs firebase-backup.test.mjs firebase-deadline.test.mjs firebase-archive.test.mjs' : 'node --test firebase-session.test.mjs'],
