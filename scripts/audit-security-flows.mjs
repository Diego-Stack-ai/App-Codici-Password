...
assert.match(homeHtml, /data-app-version>v1\.1\.4</, 'La home non mostra una versione di fallback durante l’aggiornamento cache');
...
assert.match(homeHtml, /main\.js\?v=1\.1\.4/, 'La home non forza il caricamento della release corrente');
...
assert.match(loginHtml, /main\.js\?v=1\.1\.4/, 'Il login non forza la release JavaScript corrente');
assert.match(loginHtml, /codex_shell_recovery_v114/, 'Il login non recupera automaticamente le cache PWA incoerenti');
...
console.log('Audit sicurezza e offline: 50 controlli superati.');
