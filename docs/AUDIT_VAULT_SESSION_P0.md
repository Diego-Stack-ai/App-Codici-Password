...
## Verifica isolamento e chiavi sulla base 1.2.127 — 15/09/2026

Base esaminata: release 94792d837cadd538e17aa67e439df1c68a09a23a, ramo audit/security-isolation-v127. Verifica autorizzata limitata a codice ed emulatori con identità/dati sintetici. Nessun accesso ai dati reali, nessun deploy, nessuna modifica runtime o alle Rules. Le Rules caricate negli emulatori sono quelle del repository: non è stata verificata in questo passaggio l'identità con le Rules attualmente distribuite.

Aggiunte tre regressioni in tests/firestore.profile-widgets.rules.test.mjs: lettura/scrittura/eliminazione vietate ad anonimo e UID estraneo per profilo, impostazioni sicurezza, Account personali/aziendali; enumerazione delle raccolte vietata agli estranei; ospite esplicitamente autorizzato legge soltanto il record concesso, non lo modifica e perde l'accesso dopo revoca. La revoca impedisce nuove letture dal backend, non cancella copie già ottenute o cache di un destinatario.

Esiti: 18 test Firestore superati (9 sulle Rules produttive, 9 su prototipi), 10 Storage (5 produttivi, 5 prototipo), 9 crittografia/sessione/allegati e 36 Functions/validatori. Totale 73 controlli superati. Le Functions sono testate localmente, non invocate in produzione. Non interpretare i test dei prototipi come funzionalità distribuite né questo campione come certificazione ASVS completa.

VS-P0-01 confermato direttamente da una nuova prova con materiale fittizio in tests/vault-session.test.mjs: importando la chiave di wrapping salvata nello stesso sessionStorage si decifra il payload senza Master Password. Il test caratterizza una vulnerabilità nota, NON un criterio positivo di sicurezza; al cutover deve essere sostituito con il requisito di assenza di persistenza. Precondizione dell'attacco: capacità di eseguire codice nella stessa origine o leggere lo storage della scheda, non semplice accesso anonimo alla Home. Nessuna XSS effettiva è stata dimostrata. La pulizia dei due valori al blocco/logout è verificata; la conservazione durante la sessione resta non conforme agli invarianti 4, 6 e 7 del Contratto Vault.

Azione prioritaria: integrare e collaudare la shell persistente già scelta, mantenere la chiave sbloccata soltanto in RAM, eliminare scrittura e ripristino legacy e adeguare i test senza reintrodurre wrapping. Non rimuovere isolatamente il modulo dalla navigazione multipagina. Restano da verificare configurazione effettiva delle Rules distribuite, superficie XSS/dipendenze, percorsi backend non coperti e ciclo completo dei dispositivi. Il collaudo funzionale 1.2.127 è stato confermato dall'utente; ciò non chiude VS-P0-01.
