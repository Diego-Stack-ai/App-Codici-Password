...
## 54. Note rapide nel dettaglio Account — candidata 13/09/2026

Base `80ec7837`, ramo `fix/account-inline-notes`. Editor comune caricato su richiesta dai due dettagli; testo note visibile solo se presente e comando Aggiungi/Modifica per il proprietario. Salvataggio cifrato con confronto di revisione e contenuto precedente, patch limitata a note/revision/updatedAt, aggiornamento immediato della vista dopo conferma. In caso di errore resta la bozza; dialogo dismesso su abort, blocco Vault e navigazione. I form completi conservano il proprio campo note. Vecchi Account non marcati cifrati richiedono prima un salvataggio nel form completo: nessuna migrazione parziale o nuova nota in chiaro.

Suite completa: 946 test superati, zero fallimenti; cinque nuove prove del modulo coprono contesti privato/azienda, cifratura prima del writer, conflitti, aggiunta/modifica/svuotamento, sola lettura e blocco durante cifratura. Queste prove del nuovo writer usano fixture; gli emulatori della suite mantengono i gate generali. Dopo la rifinitura delle classi del dialogo rieseguiti i cinque test e Stylelint. Versione 1.2.121 coerente con 246 riferimenti asset; 30 pagine entro i budget invariati. Nessun dato reale modificato, deploy o collaudo fisico implicito.
