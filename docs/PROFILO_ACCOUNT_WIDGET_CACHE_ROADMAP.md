...
Censimento in `docs/A3_CENSIMENTO_UTENZE.md`. Le utenze vivono **dentro** l'indirizzo padre (`userAddresses[].utilities[]`); il writer reale espone `type` e `value`, cifra **solo `value`**, assegna `utility-<uuid>` alle righe nuove e lascia al modello di lettura un'identità legacy `utility-<idIndirizzo>-legacy-<hash>` che dipende dal contenuto e dalla posizione; i collegamenti Account stanno nella riga, i riferimenti inversi sull'Account, e il QR include l'indirizzo, non la singola utenza. Nessuno schema aziendale equivalente esiste e non è stato inventato. A3-R2 completa il candidato con sorgente revocabile, vista/provider annidati sotto A2, create/update/delete, cifratura del solo `value`, doppia conferma, scarto locale e rilettura confermata. La guardia Account e le identità instabili restano fail-closed; l'inclusione QR del padre non blocca l'utenza. Collega/Cambia/Scollega inoltrano l'origine composta al servizio collegamenti esistente. Endpoint loopback, Rules candidate e Firestore Emulator sono inclusi. Prove: **11 test mirati**, emulatore **1/1**, shell **712/712**, Chrome desktop/mobile **31/31**; Edge tentato ma chiuso prima di DevTools sulla macchina di prova. Rollback: rimuovere moduli, montaggio, endpoint, overlay e prove. **A3 è completo nel laboratorio per A3-R2, non pubblicato**; restano trasporto/Rules produttivi e dispositivo fisico.
### Editor indirizzi (A2) — laboratorio 17/09/2026

Censimento in [A2_CENSIMENTO_INDIRIZZI.md](./A2_CENSIMENTO_INDIRIZZI.md). Il **privato** tiene gli indirizzi in chiaro (`type`, `address`, `civic`, `cap`, `city`, `province`, `isPrimary`) e cifra soltanto il `value` delle utenze annidate; le righe nuove ricevono `address-<uuid>`, mentre il modello di lettura sintetizza `address-legacy-<hash>` dalle righe legacy, un'identità che dipende anche dalla posizione. L'**azienda** ha la sede fissa in campi top-level (`tipoSedeLegale`, `indirizzoSede`, `civicoSede`, `capSede`, `cittaSede`, `provinciaSede`) e le sedi ripetibili in `altreSedi[]`, tutte in chiaro, con il form legacy che riscrive `sede-<indice>`, la pubblicazione sulla tessera decisa da `qrConfig.qrLegale` e dal `qr` di riga, e nessun collegamento Account sugli indirizzi.

Consegnati in questo incremento i **contratti/allowlist separati**, la preparazione e i **servizi transazionali** dei due domini con revisione propria, impronta dell'intera riga (per la sede, dell'intera famiglia di campi), ricevuta idempotente, retry, letture prima delle scritture e nessuna scrittura parziale. Guardie: un indirizzo privato con utenze o un riferimento Account non si elimina; un'identità derivata non è indirizzabile nemmeno da una richiesta a mano; una configurazione QR non risolvibile blocca l'eliminazione in fail-closed; una sede aziendale pubblicata non si elimina; la sede fissa si aggiorna campo per campo e non si cancella; utenze, collegamenti, campi sconosciuti, `qrConfig`, contatti e ogni altra chiave sono preservati. File: `private-addresses-contract.mjs`, `prepare-private-addresses.mjs`, `private-addresses-handler.mjs`, `company-addresses-contract.mjs`, `prepare-company-addresses.mjs`, `company-addresses-handler.mjs` e le rispettive suite. Prove: **14 nuove prove unitarie** (8 private, 6 azienda), `npm run test:vault-shell` **705/705**. Completamento A2-R1: sorgenti revocabili separate, vista condivisa (`addresses-editor-view.mjs`), provider e montaggio nelle due linguette Indirizzi, endpoint `applyPrivateAddressesMutation`/`applyCompanyAddressesMutation`, Rules candidate (`private-addresses-candidate-rules.mjs`, `company-addresses-candidate-rules.mjs`), suite emulatore `--profile-addresses` con transazioni reali (2/2) e scenario browser dedicato `--profile-addresses-browser` (`emulator-addresses-check.mjs`, **27 controlli per profilo** su privato e azienda) superato su Chrome 152 ed Edge 153 con **profilo desktop 1280×800 e profilo mobile 390×844 (dpr 3)** emulati via DevTools, identità di ogni esito = coppia (browser, profilo), in due esecuzioni consecutive. Difetto reale trovato dal montaggio e corretto: nella sorgente privata una riga con identità derivata restava modificabile invece che solo consultabile. Restano aperti: trasporto produttivo, applicazione autorizzata delle Rules, prova su dispositivo fisico. Rollback: rimuovere i moduli candidati, il montaggio, gli endpoint, l'overlay, lo scenario e le prove. **A2 resta in laboratorio e non concluso**; nessun deploy, bump o dato reale.

### Contatti aziendali (A1b) — laboratorio 17/09/2026

Modifica sicura dei **contatti aziendali** nella shell sperimentale, **senza convertire l'azienda nel formato privato**. Il lettore non distruttivo dello schema era già pubblicato in `7d4eace2`; questo incremento aggiunge contratto di mutazione, preparazione, sorgente revocabile, servizio transazionale, Rules candidate, editor montato e prove.

File nuovi nel laboratorio: `company-contacts-handler.mjs`, `prepare-company-contacts.mjs`, `company-contacts-editor-source.mjs`, `company-contacts-editor-view.mjs`, `company-contacts-editor-provider.mjs`, `company-contacts-candidate-rules.mjs`, `company-contacts.test.mjs`, `company-contacts-editor.test.mjs`, `firebase-company-contacts.test.mjs`. Modificati: `company-contacts-contract.mjs` (mutazione, guardie, impronta, revisione, stato QR e correzione dello svuotamento telefonico), `company-contacts-contract.test.mjs`, `emulator-entry.mjs` (montaggio nella linguetta Contatti aziendale), `emulator-qr-bridge.mjs` (endpoint `applyCompanyContactsMutation`), `emulator-entry-check.mjs`, `scripts/run-vault-session-emulators.mjs` (flag `--profile-company-contacts`) e `package.json`.

Schema rispettato alla lettera: `emails.pec`/`amministrazione`/`personale` sono oggetti con `tipo`, `email`, `note` e `password` (l'unico campo che `ma_save.js` cifra); `emails.extra` è una lista ripetibile con `id` opzionale, `qr` e campi sconosciuti; `telefonoAzienda`/`faxAzienda`/`referenteCellulare` sono **stringhe top-level**; il collegamento Account vive dentro l'oggetto email o in `phoneAccountLinks`; `aziendaEmail`/`aziendaEmailPassword` restano fallback legacy di sola consultazione, mai riscritti. Nessun ID derivato dall'indice: le nuove righe ricevono `company-email-<uuid>` e una riga legacy senza identità stabile resta visibile e non modificabile.

Guardie: uno slot fisso si **svuota** e non si cancella; lo svuotamento è rifiutato se lo slot è collegato a un Account, se la tessera digitale lo pubblica o se il valore visibile è il fallback legacy `aziendaEmail`; una riga `emails.extra` si elimina solo con ID stabile, non collegata e non pubblicata; un `qrConfig` non risolvibile (o una bandierina `qr` non booleana) blocca in fail-closed ogni svuotamento ed eliminazione senza impedire la consultazione. La semantica della tessera è quella reale di `company-vcard.js`: senza `qrConfig` PEC e cellulare referente sono pubblicati, `adminEmail`/`persEmail`/`telefonoAzienda` sono opt-in, una riga `extra` è pubblicata salvo `qr === false`.

Correzione di un difetto del contratto consegnato: `emptyCompanyContactSlot` diffondeva uno slot telefonico stringa (`{...'0110000000'}`) distruggendo il record; ora la stringa resta una stringa.

Prove: **31 nuove prove unitarie** (5 contratto/guardie, 17 preparazione/servizio/sorgente, 9 editor/interfaccia) e **2 prove emulatrici** con transazioni Firestore reali e Rules candidate; `npm run test:vault-shell` **691/691**. **Scenario browser sintetico dedicato** (`--profile-company-contacts-browser`): 22 controlli con provider, sorgente, vista, controller e servizio transazionale reale su uno store isolato in pagina — rendering con etichette aziendali, salvataggio confermato e riletto nella stessa linguetta, scarto locale senza richiesta, doppia conferma, guardie Account/QR e configurazione ambigua, offline consultativo, revoca su cambio sezione/lock/logout/cambio UID, risposta tardiva ignorata, zero errori di console — superati su **Chrome 152 ed Edge 153**, due esecuzioni consecutive, exit 0. Il montaggio ha rivelato un difetto reale della vista — l'etichetta aziendale della riga non era resa, quindi gli slot fissi erano indistinguibili — ora corretto e coperto da regressione unitaria. Limiti dichiarati: il trasporto di produzione, l'applicazione autorizzata delle Rules candidate, la prova su dispositivi e il collegamento del salvataggio al bridge reale in browser (qui il servizio gira in pagina, mentre il bridge è coperto dalla suite emulatrice e dal rifiuto anonimo verificato nello scenario) restano aperti. Rollback: rimuovere i moduli, il montaggio, l'endpoint, l'overlay, lo scenario e le prove; nessun dato, Rules o Function produttive toccati. Nessun deploy, bump o dato reale.
...
**Revisione DS-002A-R1 (17/09/2026).** Correzioni richieste dalla revisione Codex e applicate al servizio candidato: letture prima delle scritture in ogni transazione — con un fake unitario che **rifiuta read-after-write** e una nuova suite su **Firestore Emulator**, dove l'ordine è imposto da Firestore stesso; profilo autorevole letto nella prenotazione con `documentId` univocamente persistito; limite di dieci immagini deciso **atomicamente** nella transazione di prenotazione tramite lettura transazionale limitata (`attachmentsFor({uid, documentId})`), senza contatori e quindi senza deriva da compensare; copia binaria del payload e SHA-256 confrontati con `command.digest` prima della scrittura, con `storage.probe()` che impedisce di sovrascrivere un oggetto con byte diversi; cancellazione che valida l'intero record canonico e non il solo digest; `recover()` che promuove o elimina solo oggetti dimostrati dal digest registrato nella ricevuta (`objectDigest`), lasciando bloccato il resto. Contratto di trasporto candidato per DS-002B: `db = {doc, get, list, attachmentsFor, runTransaction}`, `tx = {get, listAttachments, create, update, delete}` con letture prima delle scritture, `storage = {probe, putIfAbsent, remove}`. Prove: **42 unitarie** (35 di DS-002A più 7) più la suite emulatore `npm run test:profile-document-attachments-emulators` (caricamento idempotente, documento non univoco, corsa sul decimo posto, cancellazione validata, recupero con digest). Il candidato resta **non montato**.

**Revisione DS-002A-R2 (17/09/2026).** Seconda revisione Codex: chiusura delle **finestre TOCTOU** fra verifica e scrittura. Le ricevute di caricamento e cancellazione hanno **validatori canonici** (allowlist esatta per tipo, percorso ricalcolato da proprietario/documento/allegato, digest di operazione e digest dell'oggetto, `objectSize`, stato ammesso, `readyAt`/`removedAt` solo nello stato che li possiede; `id` di trasporto tollerato solo se uguale al nome derivato) e una ricevuta non conforme resta bloccata senza toccare record o Storage. La ricevuta registra la **dimensione cifrata** (`objectSize`, misurata nel confine fidato), distinta dai byte originali del record: se il trasporto riporta la dimensione dell'oggetto deve coincidere, se non la riporta la proprietà è dimostrata dal solo digest, e una dimensione o un digest discordanti non permettono promozione, sovrascrittura o cancellazione. Finalizzazione del caricamento e della cancellazione rileggono **nella stessa transazione** record e ricevuta e riprovano l'intera coerenza con il comando (MIME, dimensione, digest, envelope campo per campo, stato) prima di scrivere: un valore cambiato dopo la prenotazione produce `FINALIZE_CONFLICT` e nulla viene promosso o cancellato. `recover()` non separa più verifica e scrittura: rilegge entrambi nella transazione finale, rimuove l'oggetto solo dopo una prova immediatamente precedente di digest e dimensione, e la compensazione non cancella mai un record non riprovato. Prove: **53 unitarie** (42 più 11) e **8 prove su Firestore Emulator**; `npm run test:vault-shell` **617/617**. Il candidato resta **non montato** e non attivo in produzione.

**DS-002B, primi due blocchi (17/09/2026).** Dettagliato da Codex e consegnato in due blocchi, sempre senza montare nulla in `Frontend/public/**`. **Blocco 1** (`0bb19c60`): sigillo **reale** AES-GCM-256 con chiave-file avvolta da HKDF-SHA256 sul materiale della Vault Key e AAD contestuale, con azzeramento di chiavi e bit derivati; la sessione espone `sealImage`/`openImage` con le stesse verifiche di `read`/`encrypt` e la capacità binaria apre ciò che ha sigillato; il trasporto porta la **versione nativa** dell'oggetto e la cancellazione è condizionata alla versione appena verificata; **adattatori reali** Firestore e Firebase Storage (`ifGenerationMatch: 0` in creazione, `probe` con digest, dimensione e versione); **Rules candidate** con trasformazioni ancorate ai file produttivi: record degli allegati leggibili dal solo proprietario e non scrivibili dal client, oggetti Storage confinati allo UID con marcatore `encrypted: 'v1'` e envelope di 10 MiB più overhead. **Blocco 2** (`c826ff3e`): sorgente revocabile della galleria (una sola anteprima, Object URL revocato e plaintext azzerato a chiusura, blocco, logout, cambio UID, annullamento e `dispose`; caricamento file per file; nessuna scrittura offline) e interfaccia con azione **Allegato** accanto a Modifica e Cestino, selezione multipla, galleria con Apri e Cancella, anteprima con Chiudi e messaggio per i documenti senza ID persistito univoco. Prove: **80 unitarie della fetta** (`npm run test:vault-shell` **644/644**), **8** su Firestore Emulator e **9** sulla suite `auth,firestore,storage` (caricamento cifrato reale end-to-end, creazione condizionale, cancellazione coordinata, recupero, corsa sul decimo posto, due prove Rules). **Riscontri:** l'**emulatore Storage non applica `ifGenerationMatch`** (la suite lo asserisce: la verifica della versione è dell'adattatore, la garanzia atomica resta da provare su Cloud Storage reale) e le **Rules Storage concedono l'accesso se una qualsiasi regola corrisponde**, quindi la regola generica del proprietario copre ancora il percorso degli allegati (serve un modello esplicito per collezione, decisione di deploy non presa). Il servizio resta fail-closed. **Non compresi**: montaggio nella pagina di laboratorio e scenario browser sintetico.

**DS-002C (17/09/2026).** Pannello **Allegati** montato nella sezione Documenti della pagina di laboratorio con un provider che riusa capacità, lettore, sorgente, vista e pianificatori di DS-002B. Due difetti della sorgente corretti su prova dei test: identità dell'allegato derivata dal contratto canonico invece che da un campo non ammesso, e pianificatore di cancellazione alimentato con il record grezzo. Scenario **browser sintetico** su **Chrome ed Edge**: 13 controlli superati (azioni accanto a Modifica e Cestino, messaggio del documento senza ID, focus da tastiera, selezione multipla, caricamento con sigillo reale, galleria aggiornata, anteprima e revoca dell'Object URL, offline in sola consultazione, cancellazione, revoca al cambio vista, nessun errore di console), su fixture e sigillo reale; il trasporto resta provato dalle suite emulatrici. Prove: `test:vault-shell` **649/649** e `npm test` completo. Restano: emulazione mobile e prova su dispositivo reale, trasporto di produzione e Rules autorizzate, precondizioni di generazione su Cloud Storage reale.

Restano aperti: trasporto callable con Auth/App Check e verifica delle precondizioni su Cloud Storage reale; applicazione autorizzata delle Rules candidate con il modello per collezione; montaggio in laboratorio e scenario browser; doppio lettore e non regressione sui dati già cifrati; cestino/retention (M7), interazione con backup/ripristino (M8) e gate §16 dell'architettura. Rischi residui: l'irrigidimento più restrittivo vive nel client finché le Rules non sono autorizzate; `recover()` dipende da un trasporto che sappia elencare la collezione delle operazioni e da ricevute canoniche (una ricevuta priva di `objectSize` o con campi estranei resta bloccata, senza migrazione implicita); quando il trasporto non riporta la dimensione dell'oggetto la proprietà è dimostrata dal solo digest; la cifratura reale è provata in laboratorio ma non su dispositivi. Rollback: rimuovere i moduli candidati, le suite e il registro in `package.json`, senza migrazioni. **Nessun dato reale, deploy, bump, modifica a master o alle Rules/Functions produttive.**
...
A5-R1 chiude il gate browser Chrome. La causa era la readiness del picker: il pulsante di creazione appariva mentre la lettura confermata e il relativo handler erano ancora pendenti, consentendo a un click anticipato di andare perso. Il comando è ora disabilitato fino alla disponibilità reale; una regressione deterministica copre `picker → callback → form`. Mirati create/picker/link **35/35**, emulatore **2/2**, shell **714/714** e suite completa superati. Chrome 152 desktop 1280×800 e mobile 390×844 completano creazione, rilettura, Apri/Cambia/Scollega, ripristino, offline e revoche per le origini ammesse. Edge desktop resta al gate ambientale `DEVTOOLS_BROWSER_EXITED_BEFORE_ENDPOINT:0` e la matrice non raggiunge il profilo mobile. A5-R1 è approvato nel laboratorio; trasporto/App Check e Rules produttivi, migrazione delle identità legacy e dispositivo fisico restano separati.
