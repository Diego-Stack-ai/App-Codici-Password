...
Prove: **31 nuove prove unitarie** (5 contratto/guardie, 17 preparazione/servizio/sorgente, 9 editor/interfaccia) e **2 prove emulatrici** con transazioni Firestore reali e Rules candidate; `npm run test:vault-shell` **691/691**. Limiti dichiarati: nessuno scenario browser dedicato ai contatti aziendali; nessun trasporto produttivo; le Rules candidate non sono autorizzate all'applicazione. Rollback: rimuovere i moduli, il montaggio, l'endpoint, l'overlay e le prove; nessun dato, Rules o Function produttive toccati. Nessun deploy, bump o dato reale.
