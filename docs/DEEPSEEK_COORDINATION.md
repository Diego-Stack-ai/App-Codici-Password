...
**Stato incarico: IN_LAVORAZIONE** — presa in carico 2026-09-17 22:28 (DeepSeek); commit osservato `0957a737`. Aggiungo identità esplicita (`chrome`/`edge`), percorso, argomenti, exit code e stderr sanificato a ogni esito; verifico perché Edge termina con 0 prima dell'endpoint con esecuzione isolata; applico una correzione limitata al runner e regressioni su doppio conteggio e risultati senza identità. Se Edge resta indisponibile dopo la diagnosi, imposto `BLOCCATO` con stderr e ambiente. Nessuna modifica alla funzionalità Allegati, alla produzione, alla versione, a `master` o al deploy.
