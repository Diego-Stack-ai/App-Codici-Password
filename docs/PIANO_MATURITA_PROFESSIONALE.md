...
2. In corso: consultazione dei Widget Account e delle credenziali comuni nella UI della shell. Dopo `8a664499` è pronto il lettore revocabile, con 15 test mirati e 278 shell finali; [perimetro e prossimi passi](./PROFILO_ACCOUNT_WIDGET_CACHE_ROADMAP.md#lettore-widget-per-la-shell--candidata-15092026). Restano collegamento UI, fixture browser non vuote e prove offline: il lettore isolato e il probe non equivalgono a UI integrata. I renderer canonici usano ancora `ensureVaultKeyMaterial`: adattare il confine di lettura senza reintrodurre il gestore della sessione legacy. Il flag `readOnly` degli Widget incorporati oggi nasconde la sezione: non confonderlo con la sola esclusione dei comandi di modifica.
