...
Prove: **31 nuove prove unitarie** (5 contratto/guardie, 17 preparazione/servizio/sorgente, 9 editor/interfaccia) e **2 prove emulatrici** con transazioni Firestore reali e Rules candidate (`--profile-company-contacts`); `npm run test:vault-shell` **691/691**. Rollback: rimuovere i moduli candidati, il montaggio, l'endpoint, l'overlay e le prove, senza toccare dati, Rules o Function produttive. **A1b resta in laboratorio e non concluso**: manca lo scenario browser dedicato ai contatti aziendali, dichiarato come scostamento nel rapporto dell'incarico. Nessun deploy, bump, master o dato reale.
