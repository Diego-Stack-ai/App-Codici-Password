...
**Consegnato nel completamento A2-R1** (commit e conteggi nel rapporto): sorgenti revocabili **separate** per i due domini, una vista condivisa, i due provider, il montaggio nelle due linguette **Indirizzi** (`mountAddressesEditor`), gli endpoint loopback `applyPrivateAddressesMutation`/`applyCompanyAddressesMutation`, le **Rules candidate** che chiudono `userAddresses` e `_profileAddresses*` da un lato e la sede, `altreSedi` e `_companyAddresses*` dall'altro, la suite emulatore con transazioni reali e lo **scenario browser dedicato** (privato e azienda, 19 controlli, desktop) superato su **Chrome 152 ed Edge 153** in due esecuzioni consecutive. Il montaggio ha rivelato un difetto reale della sorgente privata — una riga con identità derivata restava modificabile — ora corretto. **A2 resta in laboratorio e non concluso**: nessun trasporto produttivo, nessuna Rules autorizzata all'applicazione, nessuna prova su dispositivo e nessun viewport mobile realmente emulato. Nessun deploy, bump, `master`, dato reale o modifica a Rules/Functions produttive.
