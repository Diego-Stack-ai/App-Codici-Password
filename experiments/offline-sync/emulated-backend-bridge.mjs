    ...
    return {
        async handle(request, response) {
            if (request.url === '/fixture' && request.method === 'GET') {
                response.setHeader('Content-Type', 'application/json'); response.end(JSON.stringify({uid, privateAccounts})); return true;
            }
            if (!['/mutation', '/snapshot', ...(privateAccounts ? ['/scope'] : [])].includes(request.url)) return false;
            ...
            const {operation, dropResponse, scope} = JSON.parse(body);
