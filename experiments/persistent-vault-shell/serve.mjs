...
allowed.set('sw.js', 'dist/site/sw.js');
...
const server = createServer(async (req, res) => {
    ...
    try {
        ...
        res.writeHead(200, {
            'Content-Type': /\.(m?js)$/.test(name) ? 'text/javascript' : name.endsWith('.css') ? 'text/css' : name.endsWith('.woff2') ? 'font/woff2' : name.endsWith('.png') ? 'image/png' : 'text/html',
            'Content-Security-Policy': "default-src 'none'; script-src 'self'; style-src 'self'; font-src 'self'; img-src 'self'; worker-src 'self'; connect-src 'self'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'",
