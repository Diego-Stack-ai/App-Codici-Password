...
export function mountDigitalCardView(root, context, {generate, loadQr, renderQr, makePayload, download}) {
    ...
    const controls = new AbortController(); let disposed = false, busy = false;
    ...
    const run = async action => {
        if (busy) return; busy = true; create.disabled = save.disabled = true;
        ...
        finally {busy = false; if (!disposed) create.disabled = save.disabled = false;}
    };
